# Модуль, содержащий хэндлеры, обрабатывающие команды от пользователя

# Импортируем необходимые классы и методы
from aiogram import F, Router, html
import uuid
from Data.data_inline import links,photo_links #  Импортируем ссылки
...
# Хэндлер, обрабатывающий команду отображения информации на английском    
@user_router.inline_query(F.query == 'tour_info_en')
async def show_inline(inl_q:InlineQuery,i18n_en:TranslatorRunner):# Передаём в функцию объект инлайн-запроса и объект класса TranslatorRunner, содержащий информацию об описании на английском
...
# Хэндлер, обрабатывающий команду отображения информации на русском         
@user_router.inline_query(F.query == 'tour_info_ru')
async def show_inline(inl_q:InlineQuery,i18n_ru:TranslatorRunner):# Передаём в функцию объект инлайн-запроса и объект класса TranslatorRunner, содержащий информацию об описании на русском
        ...
        await inl_q.answer(id=inl_q.id,results=result,is_personal=True,cache_time=1)
# Хэндлер, обрабатывающий команду отображения информации на французском        
@user_router.inline_query(F.query == 'tour_info_fr')
async def show_inline(inl_q:InlineQuery,i18n_fr:TranslatorRunner):# Передаём в функцию объект инлайн-запроса и объект класса TranslatorRunner, содержащий информацию об описании на французском
